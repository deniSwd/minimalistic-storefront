...
const cartReducer = (state = initialsState, action: any) => {
  ...
  switch (action.type) {
   ...
   /*   const stateCopy = JSON.parse(JSON.stringify(state))
      ...
      return stateCopy*/
       const currentCartProductsForAdd: any = [...state.currentCart.selectedProducts]
       const copyOfProduct = currentCartProductsForAdd.find((v: any) => v.product.id === action.selectedProduct.product.id
       && JSON.stringify(v.currentItem) === JSON.stringify(action.selectedProduct.currentItem))
       if(copyOfProduct) ++copyOfProduct.amount
       return {
         ...state,
         currentCart: {
          ...state.currentCart,
           selectedProducts: copyOfProduct ? [...state.currentCart.selectedProducts] :
             [...state.currentCart.selectedProducts, { ...action.selectedProduct, amount: 1 }]
         }
       }
