...
const categoryPageReducer = (
  state: CategoryType = initialsState,
  action: any
) => {
  switch (action.type) {
    case SET_PRODUCTS:
...
export const setAllProductsSuccess = (
  category: CategoryType
): setAllProductsSuccessActionType => ({
  type: SET_PRODUCTS,
  category,
})
