...
export class CategoryPage extends Component<any> {
  ...
  render() {
    ...
    const renderCategory: any = this.props.products.filter(
      (u: any) => u.category === categoryName
    )
    return (
      <div>
        ...
        <div className={s.products}>
          {this.props.products && categoryName
            ...
            : this.props.products.map((u: any, i: number) => (
                <Product
                  key={i}
                  name={u.name}
                  image={u.gallery[0]}
...
const CategoryPageContainer = connect(mapStateToProps, { getAllProducts })(
  CategoryPage
)
