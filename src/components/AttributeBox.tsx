  ...
  render() {
    ...
    return <div>
      ...
      <div className={s.attributesItem}>
        {this.props.attribute.items.map((item: AttributeType) => attributeId === 'Color' ?
          <div
            className={Object.keys(currentItem).length > 0 && currentItemAttribute === item ? s.currentItemElement : s.itemElement}
            style={{ backgroundColor: item.value }} onClick={() => this.props.getProductItem(item, attributeId)}/>
          : <div
            className={Object.keys(currentItem).length > 0 && currentItemAttribute === item ? s.currentItemElement : s.itemElement}
            onClick={() => this.props.getProductItem(item, attributeId)}>{item.value}</div>)}
      </div>
    </div>
  }
}